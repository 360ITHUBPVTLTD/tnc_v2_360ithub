...
def before_update_after_submit(doc, method=None):
	...
	if not doc.get("custom_student_batch_enrollment"):
		return
	...
	if sig(doc) != sig(before):
		frappe.throw(_("This order was created by enrolment {0}. Change the fee or instalments there, not with Update Items.").format(doc.custom_student_batch_enrollment))
